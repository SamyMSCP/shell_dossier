...
class ComponentTypePaysEdit extends ComponentGenerator {

	protected static $_dependances = [];
	protected static $_componentName = "component-type-pays-edit";
	...
	protected static function getTemplate($class, $config) {
		...
		uasort($list, function($a, $b) {
			return (strcmp($a->getNomFrFr()->get(), $b->getNomFrFr()->get()));
		});
		...
		$rt .= "<option value='France'>France</option>";
		foreach ($list as $key => $elm) {
			$rt .= "<option value='{$elm->getNomFrFr()->get()}'>{$elm->getNomFrFr()->get()}</option>";
		}
		$rt .= "";
		$rt .= "</select>";
		$rt .= "
