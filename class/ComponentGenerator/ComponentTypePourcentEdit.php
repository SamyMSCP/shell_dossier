...
class ComponentTypePourcentEdit extends ComponentGenerator {

	protected static $_dependances = [];
	protected static $_componentName = "component-type-pourcent-edit";
	...
	protected static function getComponent($class, $config) {
		...
		return ("
			...
			function formatThousandPourcent(valueIn) {
				var value2 = String(valueIn).replace('.', ',');
				var value = unformatThousandPourcent(valueIn);
				...
				'$componentName',
				{
					...
					methods: {
						sendChange: function(elm) {
							this.\$emit('change', unformatThousandPourcent(elm.target.value));
						},
						setData: function(evt) {
							...
							var val = formatThousandPourcent(that.evt.target.value);
							var tmp = ('' + val).split(',');
							...
							that.data.value = unformatThousandPourcent(formatThousandPourcent(that.evt.target.value))
							that.evt.target.value = formatThousandPourcent(this.evt.target.value, 2, ' ');

							setTimeout(function() {
								that.\$emit('change', parseFloat(that.data.value));
							}, 500);
						}
					},
					computed: {
						getData: function() {
							...
							if (this.evt != null) {
								setTimeout(function() {
									var val = ('' + formatThousandPourcent(that.data.value)).length;
							...
							return formatThousandPourcent(this.data.value, 2, ' ');
						}
					},
					template: '#$templateId'
				}
