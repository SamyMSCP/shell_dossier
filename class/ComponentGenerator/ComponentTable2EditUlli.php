...
class ComponentTable2EditUlli extends ComponentGenerator {
	...
	protected static function getTemplate($class, $config) {
		...
		$rt = "<ul class='$componentClassName $componentName component'>";
		if (SHOW_FRAME)
		...
		foreach ($class::getDataTypes() as $col => $colConfig)
		{
			if (isset($colConfig['notShow']) && $colConfig['notShow'] === true)
				continue ;
			$rt .= "<li v-if='data.selected.$col.canSet' class='$col-class'>$col : ";
			...
			$rt .= "</li>";
			$rt .= "<li v-else class='$col-class'>$col : ";
		...
		$rt .= " <li> <button @click='save()'>Enregistrer</button> </li> ";
		$rt .= " <li> <button @click='set_new()'>Nouveau</button> </li> ";
		$rt .= "</ul>";
		return ($rt);
	}
