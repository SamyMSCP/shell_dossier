...
///////////////////////////////////////////
	...
	public static function checkCheckSituation($instances, $store) {
		...
		/*
		if ($situation != null && $situation->getIdSituation()->get() == $projet->getSituationPhysique()->get()->getIdSituation()->get()) // Si une situation valide déja définit
		{
		...
		else if ($situation != null && $situation->getIdSituation()->get() != $projet->getSituationPhysique()->get()->getIdSituation()->get()) // Si une situation valide déja définit
		{
			$store->setDispatch("modal_message_box", [
				"content" => "Le béneficiaire de ce projet à été changé !"
			]);
...
/*
		...
		if ($situationProjet != null) // Le projet à déja une situation d'enregistrée
		{
			// il faut vérifier que le béneficiaire de cette situation est bien le même que celui du projet;
			$situationProjet
		}
		else 
		*/
		...
		*/
		if ($situation !== null && $situation->getIsValid()->get()) // Si on en a et qu'elle est complete
		{
		/*
			$store->setDispatch("modal_message_box", [
				"content" => "Trouvé une situation complete, il faut développer la modale qui demande si la situation a changée, ici"
			]);
			$store->error();
			*/
			// On regarde dans form si on a eu une réponse pour la modale suivante !
			if (1) // Si on a la réponse 
			{
				if (1) // Si il confirme
				{
					// on copie la situation la plus récent et on la définit comme complete et on l'utilise!
					// Passe au profil d'investisseur directement !
				}
				// on copie la situation la plus récent et on la définit comme non complete et on l'utilise!
			}

			// On la présente à l'utilisateur et on lui demande si n'y a pas eu de changement dans une modale.
			...
			/*
			$store->setDispatch("modal_message_box", [
				"content" => "Trouvé une situation complete"
			]);
			*/
			return (1);
		}
