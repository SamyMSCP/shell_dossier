...
$url = "http://".$_SERVER['SERVER_NAME']."/OrderApi.php";
$data = array("p" => "scpi/tab/img", "type" => "0", "id" => $scpi);
$login = "admin";
$passwd = "preprod!2042";
$auth = "Authorization: Basic ". base64_encode("$login:$passwd") . "\r\n";
...
$img_a = file_get_contents($url, false, $context);

$data['type'] = 1;
$option['http']['content'] = http_build_query($data);
$context = stream_context_create($option);
$img_v = file_get_contents($url, false, $context);

ob_start();
//SERVER_NAME is for get the current HostName. 'cause can't get the img without this.
?>
	<img  class="img-tab" src="<?= $img_a ?>"/>
<?php
$b = ob_get_contents();
ob_get_clean();
...
$data = array("both_enable" => "0", "p" => "scpi");
...
?>
	...
	<page backimg="module/ValeurOrdres/img/MS-watermark.png" backtop="30mm" backbottom="85mm" footer="page">
		...
		<br/>
		<p>
			Les éléments inclus dans ce document reposent sur des informations obtenues par MeilleureSCPI.com auprès des sociétés de gestion et d'autres sources que MeilleureSCPI.com considère comme raisonnablement fiables. MeilleureSCPI.com ne saurait être tenu pour responsable de toute erreur, ni ne garantit l'exactitude de ces informations, fournies à titre indicatif.
		</p>
		<p>
			En l'&eacute;tat actuel du march&eacute;, si vous souhaitez vous positionner sur le march&eacute; secondaire:<br/><br/>
		<div class="tac info-container">
			<table border="1">
				...
				<tr>
					...
					<td class="">Prix conseill&eacute; Acqu&eacute;reur:</td>
					<td class="2"><?=number_format($prix_aquereur,2,",", " ");?> &euro;</td>
				</tr>
				<tr>
					<td>Valeur reconstitution au <?=$date_table?>**</td>
					<td class="dk-bg"><?=number_format($scpi_obj->getValeurReconstitution(), 2, ",", " ")?> &euro;</td>
					<td>TDVM***</td>
