	...
	<tbody>
		...
		<tr style="background-color: #01528A;" class="alignChildMiddle">
			...
			if ($keyType != "Pleine") {
				...
				<th  onmouseover="display_tooltip_table_head('Durée', 'Durée en nombre d\'années du démembrement ',event)" onmouseout="disable_msg(event)" style="cursor:help;">Fin Jouissance</th>
				<?php
			}
			?>
		...
		<tr class="toSlide toSlide_<?=$key?>">
			...
			<td style="position:relative;">
				...
				?>
					...
					<span style='cursor:pointer; color: #01528A; text-decoration: underline;' onclick="ft_change_at_input(this, <?=$buy["buy"]->id?>, 'date', 42)">Renseigner une valeur ?</span>
				<?php
				}
				?>
			</td>
			...
			if ($buy["buy"]->getTypePro() != "Pleine propriété") {
				?>
				<td ><?=$buy["buy"]->getClefRepartition()?> %</td>
				<td><?=$buy["buy"]->getDemembrement()?></td>
				<td><?=$buy["buy"]->getDateTimeSortieJouissance()->format("d/m/Y")?>
				<?=$buy["buy"]->getNombreJourRestant()?></td>
				<?php
			}
			?>
										...
										prepareSellPart(
											...
											'<?php
												if (strstr($buy['buy']->getTypePro(), "Nue"))
													echo $buy['buy']->getEndDemembrement()->format('Y-m-d');
												else
...
<?=$dataTmp?>
				...
				<div class="modal-footer">
					<button name="singlebutton" class="btn btn-primary update_info">Valider</button>
