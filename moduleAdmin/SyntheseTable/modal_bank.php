<div <?php echo 'class="modal fade modal_bank_' . $tab[$i]['id'] . '"'; ?> tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
   <div class="modal-dialog modal-std">
		<div class="modal-content">
			...
			<tbody>
				<?php $doc = get_doc_by_id($tab[$i]['lien_bank']);
				$tlen = count($doc);
				for ($c=0; $c < $tlen; $c++){ 
					?><tr>
						<td><?php echo ($c + 1); ?></td>
			 ...
			 <div style="background-color : #C0C0C0; text-align: center;"><h2>Envoyer un document</h2></div>
						<div class="row">
							<div class="col-lg-2">
								<img <?php echo 'onclick="$(\'.modal_bank_' . $tab[$i]['id'] . '\').modal(\'show\')"'; ?> src="<?=$this->getPath()?>img/bank.svg" style="cursor:pointer; width: 100px;">
							</div>
							<div class="col-lg-3">
								<h2>Offre de pret<h2>
							</div>
							<div class="col-lg-3">
								<input style="margin-top: 33px;" type="file" accept=".pdf" <?php echo $val = 'name="bank_' .  $tab[$i]['id'] . '"'; ?> >
							</div>
