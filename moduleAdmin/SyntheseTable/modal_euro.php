<div <?php echo 'class="modal fade modal_euro_' . $tab[$i]['id'] . '"'; ?> tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
   <div class="modal-dialog modal-std">
		<div class="modal-content">
			...
			<tbody>
				<?php $doc = get_doc_by_id($tab[$i]['lien_euro']);
				$tlen = count($doc);
				for ($c=0; $c < $tlen; $c++) { 
					?><tr>
						<td><?php echo ($c + 1); ?></td>
						...
						<td><?php if ($doc[$c]['signe'])
									echo '<img src="<?=$this->getPath()?>img/success.ico" alt="signé" style="width: 26px;">';
								  else
									echo '<img src="<?=$this->getPath()?>img/wait.ico" alt="non signé" style="width: 26px;">'; ?></td>
			...
			<div class="row">
							<div class="col-lg-2">
								<img <?php echo 'onclick="$(\'.modal_euro_' . $tab[$i]['id'] . '\').modal(\'show\')"'; ?> src="<?=$this->getPath()?>img/euro.ico" style="cursor:pointer; width: 100px;">
							</div>
							<div class="col-lg-3">
								<h2>BS ou OA<br>DR ou OV</h2>
							</div>
							<div class="col-lg-3">
								<input style="margin-top: 33px;" type="file" accept=".pdf" <?php echo $val = 'name="euro_' .  $tab[$i]['id'] . '"'; ?> >
							</div>
