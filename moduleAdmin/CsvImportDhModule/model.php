...
@mkdir("../cache/importsCsv/dh/old/", 0777, true);
...
if (count($_FILES)) {
	$target_file = "../cache/importsCsv/dh/BddDh.csv";
	if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file))
	{
		$vue = 1;
		setcookie("separateur", $_POST['separateur']);
		$this->data = ParseCsv::getDhCsvData("../cache/importsCsv/dh/BddDh.csv", $_POST['separateur']);
	}
	else
	{
		$vue = -1;
	...
	$this->newData = unserialize(file_get_contents(
		"../cache/importsCsv/dh/old/" . $GLOBALS['GET']['oldImport']
	));
}
...
else if (isset($_POST['what']) && $_POST['what'] == "Valider")
{
	...
	file_put_contents (
		"../cache/importsCsv/dh/old/" . $dateNow,
...
{
	$vue = 1;
	$this->data = ParseCsv::getDhCsvData("../cache/importsCsv/dh/BddDh.csv", $_COOKIE['separateur']);
}
