...
/*
if(!isset($GLOBALS['GET']['beneficiaire']))
{
	header("Location: ?p=" . $GLOBALS['GET']['p']);
	exit();
}

$this->ben = beneficiaire::getFromId($GLOBALS['GET']['beneficiaire']);

if (count($this->ben) == 0 || $this->ben[0]->getDh() != $this->dh)
{
	// Si le beneficiaire n'a pas pu etre trouve ou que que celui ci n'appartien pas a ce Dh
	Notif::set("addProject", "Le beneficiaire demande n'a pas ete trouve");
	header("Location: ?p=" . $GLOBALS['GET']['p']);
	exit();
}
*/
...
//	if (!$dh->lettreMissionOkay()) //Check si il y a bien une lettre de mission en cours de validite
	...
	else
	{
		...
		$id_crm = Crm2::insertNew(
			...
			time(),
			-2700,
			"Ce donneur d'ordre a créé un nouveau projet d'investissement pour le bénéficiaire : " . $this->beneficiaire->getShortName(),
